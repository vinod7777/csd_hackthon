...
// Require admin login (manage_ps.php handles login)
...
// Total participants = team members + team leads
$totals['participants'] = $members_count + $leads_count;

// Recent submissions for preview
$recent = [];
...
// Fetch teams for table with member counts
$teams = [];
...
<a class="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-primary/20 text-white border border-primary/20 shadow-[0_0_15px_rgba(68,60,104,0.3)]" href="admin_dashboard.php">
...
<div class="bg-surface-dark p-6 rounded-xl border border-border-dark/30 shadow-lg hover:shadow-xl transition-shadow relative overflow-hidden group">
<div class="absolute right-0 top-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
<span class="material-symbols-outlined text-8xl text-white">group</span>
</div>
<div class="relative z-10">
<p class="text-sm font-medium text-text-muted    uppercase tracking-wider mb-1">Total Teams</p>
...
<div class="relative z-10">
<p class="text-sm font-medium text-text-muted uppercase tracking-wider mb-1">Active Participants</p>
...
<div class="relative z-10">
<p class="text-sm font-medium text-text-muted uppercase tracking-wider mb-1">Problem Statements</p>
<h3 class="text-3xl font-bold text-white"><?php echo $totals['ps_active']; ?>/<?php echo $totals['ps_total']; ?></h3>
<div class="mt-4 flex items-center text-sm text-white">
...
<p class="text-sm leading-relaxed"><span class="font-medium">Teams Registered:</span> <?php echo $totals['teams']; ?> teams are actively participating</p>
<p class="text-sm leading-relaxed"><span class="font-medium">Submissions:</span> <?php echo $totals['submissions']; ?> submissions received (Status: <?php echo $totals['submissions_open'] ? ' Open' : ' Closed'; ?>)</p>
<p class="text-sm leading-relaxed"><span class="font-medium">Problem Statements:</span> <?php echo $totals['ps_active']; ?> out of <?php echo $totals['ps_total']; ?> are active</p>
<p class="text-sm leading-relaxed"><span class="font-medium">Active Participants:</span> <?php echo $totals['participants']; ?> members total</p>
</div>
</div>
...
<button class="mt-4 w-full py-2 text-sm text-white hover:text-white font-medium border border-border-dark/30 hover:bg-primary/10 rounded-lg transition-colors">
Refresh Activity
</button>
</div>
</div>
...
<thead>
<tr class="bg-primary/5 border-b border-border-dark/30 text-xs uppercase tracking-wider text-text-muted">
...
<?php if (empty($teams)): ?>
<tr><td colspan="4" class="p-4 text-text-muted">No teams registered yet.</td></tr>
<?php else: ?>
<?php foreach ($teams as $index => $team): ?>
<tr class="hover:bg-primary/5 transition-colors">
<td class="p-4 font-medium text-white"><?php echo $index + 1; ?></td>
<td class="p-4 font-medium text-white"><?php echo htmlspecialchars($team['team_name'], ENT_QUOTES); ?></td>
<td class="p-4 text-white"><?php echo htmlspecialchars($team['leader_name'], ENT_QUOTES); ?></td>
<td class="p-4 text-text-muted"><?php echo $team['member_count'] + 1; ?> (1 lead + <?php echo $team['member_count']; ?> members)</td>
</tr>
<?php endforeach; ?>
<?php endif; ?>
</tbody>
