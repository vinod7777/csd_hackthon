...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['select_ps'])) {
    ...
    // Check if PS exists and is active
    ...
    if (!$ps_exists) {
        ...
        // Check if team already selected a PS
...
// Check if problem statements are released by admin
$ps_released = false;
...
// Fetch all active problem statements with selection count (only if released)
$problem_statements = [];
if ($ps_released) {
    ...
    if ($result) {
        while ($row = $result->fetch_assoc()) {
            // Count how many teams selected this PS
...
<a class="flex items-center space-x-3 px-4 py-3 rounded-xl bg-primary/20 text-white font-semibold" href="problem_statement.php">
...
<?php if (!$ps_released): ?>
    <div class="bg-yellow-900/20 border border-yellow-500/30 rounded-xl p-8 text-center">
        ...
        <p class="text-yellow-200/80">The administrator will release the problem statements soon. Please check back later.</p>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-primary/20">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">Sno</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">Statement Name</th>
                        <th class="px-4 py-4 text-left text-xs font-bold text-white uppercase tracking-wider w-64">Description</th>
                        <th class="px-6 py-4 text-center text-xs font-bold text-white uppercase tracking-wider">Available / Total</th>
                ...
                <tbody class="divide-y divide-primary/20">
                    <?php foreach ($problem_statements as $ps): ?>
                        <tr class="hover:bg-primary/5 transition-colors <?php echo $selected_ps_id == $ps['id'] ? 'bg-primary/10' : ''; ?>">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-white font-bold"><?php echo htmlspecialchars($ps['sno'], ENT_QUOTES, 'UTF-8'); ?></span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-white font-semibold"><?php echo htmlspecialchars($ps['stmt_name'], ENT_QUOTES, 'UTF-8'); ?></span>
                            </td>
                            <td class="px-4 py-4 w-64">
                                <button type="button" onclick="openModal(<?php echo htmlspecialchars($ps['id'], ENT_QUOTES, 'UTF-8'); ?>, '<?php echo htmlspecialchars(addslashes($ps['stmt_name']), ENT_QUOTES, 'UTF-8'); ?>', '<?php echo htmlspecialchars(addslashes($ps['description']), ENT_QUOTES, 'UTF-8'); ?>')" class="text-muted-dark text-xs line-clamp-2 hover:text-primary hover:underline cursor-pointer text-left" title="Click to view full description">
                                    <?php echo htmlspecialchars(substr($ps['description'], 0, 80), ENT_QUOTES, 'UTF-8'); ?><?php echo strlen($ps['description']) > 80 ? '...' : ''; ?>
                                </button>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold <?php echo $ps['available_slots'] > 0 ? 'bg-emerald-900/40 text-emerald-300' : 'bg-red-900/40 text-red-300'; ?>">
                                    <?php echo htmlspecialchars($ps['available_slots'], ENT_QUOTES, 'UTF-8'); ?> / 20
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <?php if ($selected_ps_id == $ps['id']): ?>
                                    <span class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-semibold bg-emerald-900/40 text-emerald-200 border border-emerald-500">
                                        <span class="material-icons-outlined text-sm mr-1">check_circle</span>
                                        Selected
                                    </span>
                                <?php elseif ($selected_ps_id !== null): ?>
                                    <button type="button" disabled class="bg-gray-700 text-gray-300 px-4 py-2 rounded-lg text-sm font-semibold cursor-not-allowed opacity-60">
                                        Locked
                                    </button>
                                <?php elseif ($ps['available_slots'] <= 0): ?>
                                    <button type="button" disabled class="bg-red-900/40 text-red-300 px-4 py-2 rounded-lg text-sm font-semibold cursor-not-allowed border border-red-500">
                                        Full
                                    </button>
                                <?php else: ?>
                                    <form method="POST" class="inline">
                                        <input type="hidden" name="ps_id" value="<?php echo htmlspecialchars($ps['id'], ENT_QUOTES, 'UTF-8'); ?>"/>
                                        <button type="submit" name="select_ps" class="bg-primary hover:bg-secondary text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors">
                                            Choose
                                        </button>
                                    </form>
                                <?php endif; ?>
...
<div id="descriptionModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div class="bg-surface-dark border border-primary/30 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <div class="bg-surface-dark border-b border-primary/30 p-6 flex items-center justify-between flex-shrink-0">
            ...
            <button type="button" onclick="closeModal()" class="text-muted-dark hover:text-white transition-colors p-2 flex-shrink-0">
                <span class="material-icons-outlined text-2xl">close</span>
            </button>
        </div>
        <div class="p-6 overflow-y-auto flex-1">
            <p id="modalDescription" class="text-muted-dark text-sm leading-relaxed break-words whitespace-pre-wrap"></p>
        </div>
        <div class="bg-surface-dark border-t border-primary/30 p-6 flex justify-end gap-3 flex-shrink-0">
            <button type="button" onclick="closeModal()" class="px-6 py-2.5 rounded-lg bg-primary hover:bg-secondary text-white font-medium transition-colors whitespace-nowrap">
                Close
            </button>
        </div>
    </div>
