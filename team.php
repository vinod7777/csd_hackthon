...
// Handle add/remove team member actions
$message = '';
$message_type = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_member'])) {
        ...
        if (!empty($member_name)) {
            ...
            // Check team size limit (max 5 including leader)
            ...
            if ($current_count >= 4) { // 4 members + 1 leader = 5 total
...
<a class="flex items-center space-x-3 px-4 py-3 rounded-xl bg-primary/20 text-white font-semibold" href="team.php">
...
<!-- Team Leader Card -->
<div class="bg-surface-dark border border-primary/30 rounded-2xl p-6 shadow-sm flex flex-col md:flex-row items-center gap-6 relative overflow-hidden">
<div class="absolute top-0 right-0 p-3">
<span class="bg-primary/20 text-primary text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">Team Lead</span>
</div>
...
<input type="text" name="member_name" placeholder="Enter team member name" required class="flex-1 px-4 py-2 bg-gray-800 border border-primary/30 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" maxlength="100" <?php echo (count($team_members) >= 4) ? 'disabled' : ''; ?>/>
<button type="submit" name="add_member" class="bg-primary hover:bg-secondary text-white px-6 py-2 rounded-lg font-semibold transition-colors flex items-center gap-2" <?php echo (count($team_members) >= 4) ? 'disabled' : ''; ?>>
<span class="material-icons-outlined text-sm">add</span>
Add Member
</button>
</form>
<p class="text-xs text-muted-dark mt-2">Maximum 5 members per team (including team leader). Current: <?php echo count($team_members) + 1; ?>/5</p>
</div>
...
<?php else: ?>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <?php foreach ($team_members as $member): ?>
            <div class="bg-gray-800/50 border border-primary/30 rounded-xl p-4 flex items-center justify-between hover:border-primary/50 transition-colors">
                ...
                <form method="POST" class="ml-2">
                    ...
                    <button type="submit" name="remove_member" class="p-2 rounded-lg text-red-400 hover:text-red-300 hover:bg-red-900/20 transition-colors" onclick="return confirm('Are you sure you want to remove this team member?');">
...
if ($total_members >= 5): 
?>
    <div class="bg-yellow-900/20 border border-yellow-500/30 rounded-lg p-4 text-yellow-200 text-sm flex items-center gap-2">
