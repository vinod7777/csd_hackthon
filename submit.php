...
// Handle form submission (only when submissions are open)
...
<a class="flex items-center space-x-3 px-4 py-3 rounded-xl bg-primary/20 text-white font-semibold" href="submit.php">
...
<?php if ($existing_submission): ?>
    <div class="bg-emerald-900/20 border border-emerald-500/30 rounded-lg p-4 mb-6 flex items-start gap-3">
        ...
        <div>
            ...
            <p class="text-emerald-200/60 text-xs mt-1">Submitted: <?php echo date('F j, Y g:i A', strtotime($existing_submission['submitted_at'])); ?></p>
        </div>
    </div>
<?php endif; ?>
...
<label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all <?php echo (isset($_POST['submission_type']) && $_POST['submission_type'] === 'github') || ($existing_submission && $existing_submission['submission_type'] === 'github') ? 'border-primary bg-primary/10' : 'border-primary/30 hover:border-primary/50'; ?>">
<input type="radio" name="submission_type" value="github" class="sr-only" required <?php echo ($existing_submission && $existing_submission['submission_type'] === 'github') ? 'checked' : ''; ?>/>
...
<label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all <?php echo (isset($_POST['submission_type']) && $_POST['submission_type'] === 'drive') || ($existing_submission && $existing_submission['submission_type'] === 'drive') ? 'border-primary bg-primary/10' : 'border-primary/30 hover:border-primary/50'; ?>">
<input type="radio" name="submission_type" value="drive" class="sr-only" required <?php echo ($existing_submission && $existing_submission['submission_type'] === 'drive') ? 'checked' : ''; ?>/>
<div class="flex items-center gap-3">
...
<div>
<label class="block text-sm font-medium text-white mb-2" for="submission_link">Submission Link</label>
<input type="url" id="submission_link" name="submission_link" placeholder="<?php echo ($existing_submission && $existing_submission['submission_type'] === 'github') ? 'https://github.com/username/repository' : 'https://drive.google.com/...'; ?>" required class="w-full px-4 py-3 bg-gray-800 border border-primary/30 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" value="<?php echo htmlspecialchars($existing_submission['submission_link'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"/>
<p id="submission_hint" class="text-xs text-muted-dark mt-2">
    <span class="material-icons-outlined text-xs align-middle">info</span>
    Enter a valid <?php echo ($existing_submission && $existing_submission['submission_type'] === 'github') ? 'GitHub' : 'Google Drive'; ?> link
</p>
</div>

<button type="submit" class="w-full bg-primary hover:bg-secondary text-white font-bold py-3 px-6 rounded-lg transition-colors flex items-center justify-center gap-2">
...
<?php else: ?>
    <div class="bg-red-900/20 border border-red-500/30 rounded-lg p-4 mb-6 flex items-start gap-3">
        ...
        <div>
            ...
            <p class="text-red-200/80 text-sm">The admin has closed submissions. You cannot submit or update projects at this time.</p>
        </div>
    </div>
<?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePlaceholder(type) {
        ...
        if (type === 'github') {
            ...
            hint.innerHTML = '<span class="material-icons-outlined text-xs align-middle">info</span> Enter a valid GitHub link (e.g., https://github.com/username/repo)';
        } else if (type === 'drive') {
            linkInput.placeholder = 'https://drive.google.com/...';
            hint.innerHTML = '<span class="material-icons-outlined text-xs align-middle">info</span> Enter a valid Google Drive link (e.g., https://drive.google.com/...)';
        }
    }
    ...
    // Initialize placeholder based on checked radio or server-side value
    const checked = document.querySelector('input[name="submission_type"]:checked');
    if (checked) {
        updatePlaceholder(checked.value);
    } else {
        updatePlaceholder('<?php echo ($existing_submission && $existing_submission['submission_type'] === 'drive') ? 'drive' : 'github'; ?>');
    }
});
</script>
</body>
